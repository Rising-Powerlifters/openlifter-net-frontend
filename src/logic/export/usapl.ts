...
const addDataRow = (csv: Csv, category: Category, entry: Entry): void => {
  ...
  row[csv.index('Squat 1')] = csvString(
    displayWeight(hasSquat ? entry.squatKg[0] * entry.squatStatus[0] : 0)
  )
  row[csv.index('Squat 2')] = csvString(
    displayWeight(hasSquat ? entry.squatKg[1] * entry.squatStatus[1] : 0)
  )
  row[csv.index('Squat 3')] = csvString(
    displayWeight(hasSquat ? entry.squatKg[2] * entry.squatStatus[2] : 0)
  )
  row[csv.index('Bench 1')] = csvString(
    displayWeight(hasBench ? entry.benchKg[0] * entry.benchStatus[0] : 0)
  )
  row[csv.index('Bench 2')] = csvString(
    displayWeight(hasBench ? entry.benchKg[1] * entry.benchStatus[1] : 0)
  )
  row[csv.index('Bench 3')] = csvString(
    displayWeight(hasBench ? entry.benchKg[2] * entry.benchStatus[2] : 0)
  )
  row[csv.index('Deadlift 1')] = csvString(
    displayWeight(hasDL ? entry.deadliftKg[0] * entry.deadliftStatus[0] : 0)
  )
  row[csv.index('Deadlift 2')] = csvString(
    displayWeight(hasDL ? entry.deadliftKg[1] * entry.deadliftStatus[1] : 0)
  )
  row[csv.index('Deadlift 3')] = csvString(
    displayWeight(hasDL ? entry.deadliftKg[2] * entry.deadliftStatus[2] : 0)
  )
