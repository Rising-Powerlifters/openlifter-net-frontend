...
class LiftingTable extends React.Component<Props> {
  ...
  renderAttemptField = (
    ...
    if (
      this.props.lifting.overrideEntryId === entry.id &&
      attemptOneIndexed === this.props.attemptOneIndexed
    ) {
    ...
    const currentAndHasPrevious =
      attemptOneIndexed === this.props.attemptOneIndexed && anyPreviousAttemptTaken
  ...
  renderCell = (
    ...
    switch (columnType) {
      ...
      case 'ProjectedTotal': {
        ...
        return (
          <td key={columnType}>
            {totalKg === 0 ? null : displayWeight(asNumber, this.props.language)}
          </td>
        )
      }
      case 'ProjectedPoints': {
        ...
        if (this.props.meet.formula === 'Total') {
          return (
            <td key={columnType}>
              {points !== 0 ? displayWeight(points, this.props.language) : null}
            </td>
          )
        }
        return (
          <td key={columnType}>
            {points !== 0 ? displayPoints(points, this.props.language) : null}
          </td>
        )
      }
      case 'FinalTotal': {
